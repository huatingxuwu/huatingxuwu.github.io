...
- **Gen**: Choose a key $k \in \mathcal{B}$ uniformly at random.
- **Enc$_k(M)$**: Given plaintext $M = m_1 m_2 \dots m_n$ ($m_i \in \mathcal{B}$), compute ciphertext $C = c_1 c_2 \dots c_n$ where:
  $$c_i = m_i \oplus k \quad \forall i \in \{1, \dots, n\}$$
- **Dec$_k(C)$**: Given ciphertext $C = c_1 c_2 \dots c_n$, compute plaintext $M = m_1 m_2 \dots m_n$ where:
  $$m_i = c_i \oplus k \quad \forall i \in \{1, \dots, n\}$$

**2. Vigenère Cipher Variant (Multi-Byte XOR)**

Let $\ell$ be a fixed key length.
- **Gen**: Choose a key $k = (k_1, k_2, \dots, k_\ell) \in \mathcal{B}^\ell$ uniformly at random.
- **Enc$_k(M)$**: Given plaintext $M = m_1 m_2 \dots m_n$ ($m_i \in \mathcal{B}$), compute ciphertext $C = c_1 c_2 \dots c_n$ where:
...
**1. Attacking the Shift Cipher Variant.** The classical attack relies on a brute-force search over the key space or a frequency analysis comparison.
...
**2. Attacking the Vigenère Cipher Variant.** The attack remains a two-step process: finding the period $\ell$, then solving each stream position as an independent shift cipher.
- **Finding the Period ($\ell$)**:
  ...
  - **Index of Coincidence (IC)**: The formula for IC adapts to the larger alphabet: $\text{IC} = \sum_{i=0}^{255} \frac{f_i(f_i-1)}{n(n-1)}$, where $f_i$ is the count of byte value $i$. For an arbitrary string of English text mapped via ASCII to bytes, the expected IC is still significantly higher than that of a uniform distribution over 256 bytes ($\frac{1}{256} \approx 0.0039$). By testing stream offsets $1, 2, 3, \dots$ and extracting every $\ell$-th byte, the correct period $\ell$ will yield sub-ciphertexts with an abnormally high IC matching English text baseline properties.
...
### 1.7
#### Question
The index of coincidence method relies on a known value for the sum of the *squares* of plaintext-letter frequencies (cf. Equation (1.1)). Why would it not work using the sum $\sum_i p_i$ itself?
...
**1. Mathematically, $\sum_i p_i$ is a Constant.** By definition, the probabilities of all possible outcomes in any probability distribution must sum to 1:

$$\sum_{i} p_i = 1$$

**2. Squaring Captures "Unevenness".** To distinguish between structured language (like English) and random noise (like ciphertext), we need a metric that reacts to how heavily weighted or flat the distribution is.
...
#### Solution
Under a chosen-plaintext attack (CPA), the adversary can feed any plaintext into the encryption oracle and obtain the corresponding ciphertext. For all three ciphers, the key is recovered by exploiting the deterministic, character-by-character nature of the algorithms.
...
- **Method**: Choose the character corresponding to $0$.
  - *Modular variant*: If $m_1 = 0$, then $c_1 = (0 + k) \bmod 26 = k$.
  - *XOR variant*: If $m_1 = 0$, then $c_1 = 0 \oplus k = k$.
  - The observed ciphertext character *is* the key.
...
- **Method**: Query a single message string containing every character of the alphabet exactly once (e.g., $M = \text{"ABC...Z"}$). Because $c_i = k(m_i)$, matching the position of each character in the output gives you the exact mapping for that specific input character, fully reconstructing the permutation table $k$.
...
- **Method**: Query a string of $\ell$ zeros.
  - *Modular variant*: $c_i = (0 + k_i) \bmod 26 = k_i$.
  - *XOR variant*: $c_i = 0 \oplus k_i = k_i$.
...
- **Exact plaintexts needed**: $2\ell_{\text{max}}$ characters.
- **Method**: Query a long string consisting entirely of zeros. The resulting ciphertext will simply be the key repeated over and over ($C = k \parallel k \parallel k \dots$). The period $\ell$ is immediately visible as the exact distance at which the ciphertext characters begin to repeat. Once $\ell$ is identified from the pattern, the first $\ell$ characters of the ciphertext reveal the key.
...
**The Attack Method.** Let the ciphertext be $C = c_1 c_2 c_3 c_4$.

1. **Calculate the internal differences of the ciphertext.** The attacker computes the modular distance between consecutive ciphertext characters:

$$\Delta_1 = (c_2 - c_1) \bmod 26$$
$$\Delta_2 = (c_3 - c_2) \bmod 26$$
$$\Delta_3 = (c_4 - c_3) \bmod 26$$

2. **Compare with the candidate passwords.** The shift value $k$ cancels out when subtracting adjacent characters because $c_i = (m_i + k) \bmod 26$. Therefore, the ciphertext differences $(\Delta_1, \Delta_2, \Delta_3)$ must exactly match the plaintext character differences. Let's look at the internal differences for both possible passwords (converting letters to $0\text{--}25$):
...
#### Solution
**(a) Finding the Key Length (Equation 1.3).** The step **will still work**, but the target threshold value will change depending on the language.
...
**(b) Finding each Character of the Key (Equation 1.2).** If the attacker uses the $\{p_i\}$ vector corresponding to English text on a non-English plaintext, **the attack will fail to find the correct key characters.**
