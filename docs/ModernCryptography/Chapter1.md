...
A private-key encryption scheme consists of a procedure for generating keys ($\mathsf{Gen}$), a procedure for encrypting ($\mathsf{Enc}$), and a procedure for decrypting ($\mathsf{Dec}$).

For every key $k$ output by $\mathsf{Gen}$ and every message $m \in \mathcal{M}$, it holds that
...
**Shift cipher.** Identify the English alphabet with the set $\{0, \dots, 25\}$ (so a = 0, b = 1, etc.). The message space $\mathcal{M}$ is then any finite sequence of integers from this set. Encryption of the message $m = m_1 \cdots m_\ell$ (where $m_i \in \{0, \dots, 25\}$) using key $k$ is given by
...
Associate the letters of the English alphabet with $0, \dots, 25$. Let $p_i$, with $0 \le p_i \le 1$, denote the frequency of the $i$-th letter in normal English text (i.e., $p_0 = 0.082$ using Figure 1.3). Calculation using Figure 1.3 gives
...
When $\tau = t$ we expect $S_\tau \approx 0.065$, as discussed above. On the other hand, if $\tau$ is not a multiple of $t$ we expect that all characters will occur with roughly equal probability in the sequence $c_1, c_{1+\tau}, c_{1+2\tau}, \dots$, and so we expect $q_i \approx 1/26$ for all $i$. In this case we will obtain
